...
def query_argname(arg_name):
    ...
    def index_name(api_name, arg_name):
        arg_names = DB[signature_collection].find_one({"api": api_name})[
            "args"
        ]
    ...
    for api_name in API_args.keys():
        ...
        if (
            api_name not in DB.list_collection_names()
        ...
        if temp is None:
            ...
            if idx_name and DB[api_name].find_one(
